...
abstract class AbstractGenerator implements GeneratorInterface
{
    ...
    protected function checkProcessStatus($status, $stdout, $stderr, $command)
    {
        if (0 !== $status and '' !== $stderr) {
            if (preg_match('/Exit with code 1 due to network error: ContentOperationNotPermittedError/', $stderr)) {
              return;
            }
            ...
            if (preg_match('/Exit with code 1 due to network error: AuthenticationRequiredError/', $stderr)) {
              return;
            }
